...
#define l_popen(L,c,m)		(fsync(-1), popen(c,m))
  ...
  for (; nargs--; arg++) {
    if (lua_type(L, arg) == LUA_TNUMBER) {
      ...
      int len = lua_isinteger(L, arg)
                ...
                : fprintf(f, LUA_NUMBER_FMT, (double)lua_tonumber(L, arg));
      status = status && (len > 0);
    }
    else {
      size_t l;
...
static int io_flush (lua_State *L) {
  return luaL_fileresult(L, fsync(fileno(getiofile(L, IO_OUTPUT))) == 0, NULL);
}
...
LUAMOD_API int luaopen_io (lua_State *L) {
  ...
  createstdfile(L, luaconsolei, IO_INPUT, "stdin");
  createstdfile(L, luaconsoleo, IO_OUTPUT, "stdout");
  createstdfile(L, luaconsoleo, NULL, "stderr");
  return 1;
}
